...
describe('rewardables', () => {
  ...
  scenario('creates a rewardable', async (scenario: StandardScenario) => {
    const result = await createRewardable({
      input: {
        updatedAt: '2022-11-15T05:25:53.669Z',
        name: 'String',
        slug: 'String207801',
        explanation: 'String',
        type: 'PUZZLE',
    ...
    expect(result.updatedAt).toEqual(new Date('2022-11-15T05:25:53.669Z'))
    expect(result.name).toEqual('String')
    expect(result.slug).toEqual('String207801')
  ...
  scenario('updates a rewardable', async (scenario: StandardScenario) => {
    ...
    const result = await updateRewardable({
      id: original.id,
      input: { updatedAt: '2022-11-16T05:25:53.669Z' },
    })

    expect(result.updatedAt).toEqual(new Date('2022-11-16T05:25:53.669Z'))
  })
